...
"""
from services.senuto import api_senuto
import datetime
import requests
import pandas as pd
import openpyxl
...
def generate_audit(domain):
    ...
    api_senuto.get_important_keywords_export(domain, 'temporary_data')
    df_keywords = pd.read_excel(f'temporary_data/{domain}_keywords.xlsx')
    ...
    api_senuto.get_range_compare_export(domain, "temporary_data", get_date()[2], get_date()[0])
    df_5 = pd.read_excel(f'temporary_data/{domain}_decreased_keywords.xlsx')
    df_5.set_index(['Słowo kluczowe'], inplace=True)
    df_5.to_excel(writer, sheet_name='Wypadły z TOP10')
    api_senuto.get_range_compare_export(domain, 'temporary_data', get_date()[0], get_date()[2])
